...
def Filter_Profiles(request):
        ...
        game_profiles = GameProfile.objects.filter(game__contains=pref_game, region__contains= pref_region)
        ...
        if(pref_game == "Valorant"):
            ...
            if( rank_order ==0):
                ...
                next_rank= [k for k, v in GameProfile.Valorant_Ranks_Order.items() if v == rank_order+1][0]

            elif rank_order==GameProfile.Valorant_Ranks_Order["Max_Rank"]:
                prev_rank= [k for k, v in GameProfile.Valorant_Ranks_Order.items() if v == rank_order-1][0]
                next_rank= searched_rank
            elif rank_order>0 and rank_order<GameProfile.Valorant_Ranks_Order["Max_Rank"]:
                prev_rank=[k for k, v in GameProfile.Valorant_Ranks_Order.items() if v == rank_order-1][0]
                next_rank=[k for k, v in GameProfile.Valorant_Ranks_Order.items() if v == rank_order+1][0]
            ...
            if( rank_order ==0):
                prev_rank= searched_rank
                next_rank= [k for k, v in GameProfile.COD_Ranks_Order.items() if v == rank_order+1][0]

            elif rank_order==GameProfile.COD_Ranks_Order["Max_Rank"]:
                prev_rank= [k for k, v in GameProfile.COD_Ranks_Order.items() if v == rank_order-1][0]
                next_rank= searched_rank
            elif rank_order>0 and rank_order<GameProfile.COD_Ranks_Order["Max_Rank"]:
                prev_rank=[k for k, v in GameProfile.COD_Ranks_Order.items() if v == rank_order-1][0]
                next_rank=[k for k, v in GameProfile.COD_Ranks_Order.items() if v == rank_order+1][0]
            ...
            if( rank_order ==0):
                prev_rank= searched_rank
                next_rank= [k for k, v in GameProfile.LOL_Ranks_Order.items() if v == rank_order+1][0]

            elif rank_order==GameProfile.LOL_Ranks_Order["Max_Rank"]:
                prev_rank= [k for k, v in GameProfile.LOL_Ranks_Order.items() if v == rank_order-1][0]
                next_rank= searched_rank
            elif rank_order>0 and rank_order<GameProfile.LOL_Ranks_Order["Max_Rank"]:
                prev_rank=[k for k, v in GameProfile.LOL_Ranks_Order.items() if v == rank_order-1][0]
                next_rank=[k for k, v in GameProfile.LOL_Ranks_Order.items() if v == rank_order+1][0]
            ...
            if( rank_order ==0):
                    prev_rank= searched_rank
                    next_rank= [k for k, v in GameProfile.CS_Ranks_Order.items() if v == rank_order+1][0]

            elif rank_order==GameProfile.CS_Ranks_Order["Max_Rank"]:
                prev_rank= [k for k, v in GameProfile.CS_Ranks_Order.items() if v == rank_order-1][0]
                next_rank= searched_rank
            elif rank_order>0 and rank_order<GameProfile.CS_Ranks_Order["Max_Rank"]:
                prev_rank=[k for k, v in GameProfile.CS_Ranks_Order.items() if v == rank_order-1][0]
                next_rank=[k for k, v in GameProfile.CS_Ranks_Order.items() if v == rank_order+1][0]
        ...
        for g in game_profiles:
            ...
            if servers_present and ((prev_rank !=searched_rank and g.rank == prev_rank) or (next_rank!= searched_rank and g.rank == next_rank)):
                if(queried_profile):
                    print(queried_profile[0].bio)
                    obj = {'username': g.user.username, 'game': g.game, 'rank': g.rank, 'region': g.region,
                            'bio': queried_profile[0].bio, 'profile_pic': str(queried_profile[0].profile_pic), 'user_status': g.user_status}
